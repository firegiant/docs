...
- Every XML file has a [root element](https://en.wikipedia.org/wiki/Root_element) and for WiX source files, that root element is [Wix](https://wixtoolset.org/docs/schema/wxs/wix/).
...
The WiX element that describes an MSI package is...wait for it...[`Package`](https://wixtoolset.org/docs/schema/wxs/package/). `Package` has attributes and child elements that define everything about an MSI package.
