...
[Our witty and charming friends at FireGiant](https://www.firegiant.com/services/) have told us that we need to use the `QueryNativeMachine` element available in the `WixToolset.Util.wixext` extension. The [very brief documentation](/wix/tools/wixext/wininfo/#querynativemachine) tells us that `QueryNativeMachine` sets a property named `WIX_NATIVE_MACHINE` to:
